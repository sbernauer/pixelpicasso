...
int net_frame_to_net_frame(struct net_frame* ret, struct img_frame* src, unsigned int width, unsigned int height, bool monochrome, unsigned int offset_x, unsigned int offset_y, unsigned int sparse_perc, unsigned int penId) {
		...
		if (x % 2 == 0) {
			while(y < height - 1) {
				...
				while(true) {
					...
					print_size = snprintf(data + offset, data_alloc_size - offset, "MOVE %d 0 1 %6x\n", penId, pixel.abgr >> 8);
					if(print_size < 0) {
						err = -EINVAL;
			...
			while(y > 0) {
				...
				while(true) {
					max_print_size = data_alloc_size - offset;
					pixel = src->pixels[y * width + x];
					print_size = snprintf(data + offset, data_alloc_size - offset, "MOVE %d 0 2 %6x\n", penId, pixel.abgr >> 8);
					if(print_size < 0) {
						err = -EINVAL;
						goto fail_data_alloc;
					}
				...
				while(true) {
					max_print_size = data_alloc_size - offset;
					pixel = src->pixels[y * width + x];
					print_size = snprintf(data + offset, data_alloc_size - offset, "MOVE %d 1 0 %6x\n", penId, pixel.abgr >> 8);
					if(print_size < 0) {
						err = -EINVAL;
						goto fail_data_alloc;
					}
	...
	while(x > 0) {
		...
		while(true) {
					max_print_size = data_alloc_size - offset;
					pixel = src->pixels[y * width + x];
					print_size = snprintf(data + offset, data_alloc_size - offset, "MOVE %d 2 0 %6x\n", penId, pixel.abgr >> 8);
					if(print_size < 0) {
						err = -EINVAL;
						goto fail_data_alloc;
					}
	...
	for (int i = 0; i < 2; i++) {
		...
		while(true) {
					max_print_size = data_alloc_size - offset;
					print_size = snprintf(data + offset, data_alloc_size - offset, "MOVE %d 2 0 000000\n", penId);
					if(print_size < 0) {
						err = -EINVAL;
						goto fail_data_alloc;
					}
